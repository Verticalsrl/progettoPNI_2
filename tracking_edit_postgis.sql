...
--DROP TABLE pni_grosseto.underground_route_history;
...
CREATE TABLE pni_grosseto.underground_route_history ( like  pni_grosseto.underground_route, hid serial PRIMARY KEY, created timestamp without time zone, created_by character varying(64), deleted timestamp without time zone, deleted_by character varying(64));
GRANT ALL ON TABLE pni_grosseto.underground_route_history TO operatore;
GRANT ALL ON TABLE pni_grosseto.underground_route_history_hid_seq TO operatore;
...
INSERT INTO pni_grosseto.underground_route_history (gidd, geom, core_mater, ebw_flag_i, diameter, base_mater, ebw_tipo_p, 
...
ebw_codice, surroundin, num_fibre, owner, ripristino, measured_l, 
core_mat_1, ebw_flag_o, lungh_infr, ebw_flag_1, surface_ma, notes_1, 
width, num_mtubi, now(), current_user FROM pni_grosseto.underground_route;
...
CREATE OR REPLACE FUNCTION pni_grosseto.underground_route_insert() RETURNS trigger AS
$$
  BEGIN
    INSERT INTO pni_grosseto.underground_route_history
...
$$
LANGUAGE plpgsql;
CREATE TRIGGER underground_route_insert_trigger AFTER INSERT ON pni_grosseto.underground_route FOR EACH ROW EXECUTE PROCEDURE pni_grosseto.underground_route_insert();

--For a deletion, we just mark the currently active history record (the one with a NULL deletion time) as deleted:
CREATE OR REPLACE FUNCTION pni_grosseto.underground_route_delete() RETURNS trigger AS
$$
  BEGIN
    UPDATE pni_grosseto.underground_route_history
...
$$
LANGUAGE plpgsql;
CREATE TRIGGER underground_route_delete_trigger AFTER DELETE ON pni_grosseto.underground_route FOR EACH ROW EXECUTE PROCEDURE pni_grosseto.underground_route_delete();

--For an update, we first mark the active history record as deleted, then insert a new record for the updated state:
CREATE OR REPLACE FUNCTION pni_grosseto.underground_route_update() RETURNS trigger AS
$$
  BEGIN
    UPDATE pni_grosseto.underground_route_history
      SET deleted = current_timestamp, deleted_by = current_user
...
$$
LANGUAGE plpgsql;
CREATE TRIGGER underground_route_update_trigger AFTER UPDATE ON pni_grosseto.underground_route FOR EACH ROW EXECUTE PROCEDURE pni_grosseto.underground_route_update();