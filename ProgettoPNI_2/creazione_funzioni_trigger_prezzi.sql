...
BEGIN
  --Delete prezzi con NEW.gidd--
  EXECUTE FORMAT('DELETE FROM %s.elenco_prezzi_layer as epl WHERE epl.laygidd = NEW.gidd;', schemaDB_da_trigger);

  IF NEW.constructi = 'Progettato' THEN
    EXECUTE FORMAT('DELETE FROM %s.tab_nodo_ottico as tno WHERE tno.gidd_pozzetto = NEW.gidd;', schemaDB_da_trigger);
